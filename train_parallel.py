...
def run(dataloader, model_config, args, region, rank):
    ...
    model = GRUD(model_config, rank).to(rank)
    ...
    if args.resume and os.path.exists(check_point_dir):
        ...
        checkpoint = torch.load(filename, map_location=torch.device(rank))
    ...
    for epoch in range(start_epochs, epochs+1):
        ...
        train_loss, r2_train = train(train_loader, model, rank, loss, optimizer, scheduler)
        val_loss, r2_val, _ = evaluation(val_loader, model, rank, loss)
        ...
        if val_loss < best_val_loss:
            ...
            best_epoch = epoch
            save_model(model.module, region, output_model_dir, best=True)
...
def main(rank, world_size, set_data, model_config, args, region):
    train_set, val_set = set_data[0], set_data[1]
    ...
    run(dataloader, model_config, args, region, rank)
...
if __name__ == "__main__":
    ...
    for region in range(int(regions[0]), int(regions[-1])+1):
        ...
        mp.spawn(
            main,
            args=[world_size, [train_set, val_set], model_config, args, region],
            nprocs=world_size
        )
