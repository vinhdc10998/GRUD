...
def compute(region, args, device):
...
def main():
    ...
    print(f"[GRUD - Genotype Imputation] Chromosome: {chromosome}; Batch size: {batch_size}, Region: {args.regions}, Thread: {thread}")
    device = get_device(args.gpu)

    regions = list(range(int(regions[0]), int(regions[-1])+1))
    with multiprocessing.Pool(processes=thread) as pool:
        pool.starmap(compute, zip(regions, repeat(args), repeat(device)))
